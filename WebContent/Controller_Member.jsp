...
<%
	...
	// 카카오 로그인 유저 로그인 할 때 사용
		...
		if(memberdao.kakaoMatchIDDB(kakaomemberlogindto)==true){
			...
			url = "https://kauth.kakao.com/oauth/logout?client_id="+key+"&logout_redirect_uri=";
	...
	// 일반 회원, 카카오 회원 로그아웃 할 때 사용
		...
		if(session.getAttribute("token")==null){
			...
			// 그리고 인덱스 페이지로 가게함
			response.sendRedirect("https://kauth.kakao.com/oauth/logout?client_id="+key+"&logout_redirect_uri=");
		}
	}
